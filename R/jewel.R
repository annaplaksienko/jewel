...
#' @param stability if stability = TRUE, stability selection procedure to reduce the number of false positives will be applied. \code{n_k / 2} samples are randomly chosen in each dataset \code{stability_nsubsets} times and then __jewel__ method is applied to each subset. In the final estimate, we include only the edges that appear in at least \code{stability_frac} proportion of the subsets. By default this procedure is disabled since it increases the running time.
#' @param stability_nsubsets an optional number, how many times to subsample datasets and apply __jewel__ for stability selection procedure. The default value is 25.
#' @param stability_frac an optional number, in what proportion of the stability results on subsampled data an edge has to be present to be included into the final estimate. The default value is 0.8.
